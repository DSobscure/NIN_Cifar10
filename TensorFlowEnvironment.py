...
def CreateNetwork(inputs, keepProbe):
    ...
    convolution1Weights = CreateWeightVariable([5, 5, 3, 192], math.sqrt(2/(5*5*192)))
    convolution1Bias = CreateBiasVariable([192])
    #mlp1_1Weights = CreateWeightVariable([1, 1, 192, 160])
    mlp1_1Weights = CreateWeightVariable([1, 1, 192, 160], math.sqrt(2/(1*1*160)))
    mlp1_1Bias = CreateBiasVariable([160])
    #mlp1_2Weights = CreateWeightVariable([1, 1, 160, 96])
    mlp1_2Weights = CreateWeightVariable([1, 1, 160, 96], math.sqrt(2/(1*1*96)))
    mlp1_2Bias = CreateBiasVariable([96])

    #convolution2Weights = CreateWeightVariable([5, 5, 96, 192])
    convolution2Weights = CreateWeightVariable([5, 5, 96, 192], math.sqrt(2/(5*5*192)))
    convolution2Bias = CreateBiasVariable([192])
    #mlp2_1Weights = CreateWeightVariable([1, 1, 192, 192])
    mlp2_1Weights = CreateWeightVariable([1, 1, 192, 192], math.sqrt(2/(1*1*192)))
    mlp2_1Bias = CreateBiasVariable([192])
    #mlp2_2Weights = CreateWeightVariable([1, 1, 192, 192])
    mlp2_2Weights = CreateWeightVariable([1, 1, 192, 192], math.sqrt(2/(1*1*192)))
    mlp2_2Bias = CreateBiasVariable([192])

    #convolution3Weights = CreateWeightVariable([3, 3, 192, 192])
    convolution3Weights = CreateWeightVariable([3, 3, 192, 192], math.sqrt(2/(3*3*192)))
    convolution3Bias = CreateBiasVariable([192])
    #mlp3_1Weights = CreateWeightVariable([1, 1, 192, 192])
    mlp3_1Weights = CreateWeightVariable([1, 1, 192, 192], math.sqrt(2/(1*1*192)))
    mlp3_1Bias = CreateBiasVariable([192])
    #mlp3_2Weights = CreateWeightVariable([1, 1, 192, 10])
    mlp3_2Weights = CreateWeightVariable([1, 1, 192, 10], math.sqrt(2/(1*1*10)))
    mlp3_2Bias = CreateBiasVariable([10])

    # hidden layers convolution1
    convolution1 = TensorFlow.nn.relu(TensorFlow.contrib.layers.batch_norm(Convolution2D(inputs, convolution1Weights, 1, True) + convolution1Bias, center=True, scale=True, is_training=True))
    #convolution1 = TensorFlow.nn.relu(Convolution2D(inputs, convolution1Weights, 1, True) + convolution1Bias)
    mlp1_1 = TensorFlow.nn.relu(TensorFlow.contrib.layers.batch_norm(Convolution2D(convolution1, mlp1_1Weights, 1, False) + mlp1_1Bias, center=True, scale=True, is_training=True))
    #mlp1_1 = TensorFlow.nn.relu(Convolution2D(convolution1, mlp1_1Weights, 1, False) + mlp1_1Bias)
    mlp1_2 = TensorFlow.nn.relu(TensorFlow.contrib.layers.batch_norm(Convolution2D(mlp1_1, mlp1_2Weights, 1, False) + mlp1_2Bias, center=True, scale=True, is_training=True))
    ...
    convolution2 = TensorFlow.nn.relu(TensorFlow.contrib.layers.batch_norm(Convolution2D(convolution1Pool, convolution2Weights, 1, True) + convolution2Bias, center=True, scale=True, is_training=True))
    #convolution2 = TensorFlow.nn.relu(Convolution2D(convolution1Pool, convolution2Weights, 1, True) + convolution2Bias)
    mlp2_1 = TensorFlow.nn.relu(TensorFlow.contrib.layers.batch_norm(Convolution2D(convolution2, mlp2_1Weights, 1, False) + mlp2_1Bias, center=True, scale=True, is_training=True))
    #mlp2_1 = TensorFlow.nn.relu(Convolution2D(convolution2, mlp2_1Weights, 1, False) + mlp2_1Bias)
    mlp2_2 = TensorFlow.nn.relu(TensorFlow.contrib.layers.batch_norm(Convolution2D(mlp2_1, mlp2_2Weights, 1, False) + mlp2_2Bias, center=True, scale=True, is_training=True))
    ...
    convolution3 = TensorFlow.nn.relu(TensorFlow.contrib.layers.batch_norm(Convolution2D(convolution2Pool, convolution3Weights, 1, True) + convolution3Bias, center=True, scale=True, is_training=True))
    #convolution3 = TensorFlow.nn.relu(Convolution2D(convolution2Pool, convolution3Weights, 1, True) + convolution3Bias)
    mlp3_1 = TensorFlow.nn.relu(TensorFlow.contrib.layers.batch_norm(Convolution2D(convolution3, mlp3_1Weights, 1, False) + mlp3_1Bias, center=True, scale=True, is_training=True))
    #mlp3_1 = TensorFlow.nn.relu(Convolution2D(convolution3, mlp3_1Weights, 1, False) + mlp3_1Bias)
    mlp3_2 = TensorFlow.nn.relu(TensorFlow.contrib.layers.batch_norm(Convolution2D(mlp3_1, mlp3_2Weights, 1, False) + mlp3_2Bias, center=True, scale=True, is_training=True))
